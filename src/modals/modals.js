(function() {
    ...
    var modals = angular.module('fm.modals', ['ui.bootstrap']);

    modals.provider('fmModalsConfig', fmModalsConfig);
    function fmModalsConfig() {
        var provider = this;
        ...
        var config = {
            info: {
                templateUrl: 'fm/modals/template/info.html',
                controllerName: 'fmModalsCommonController',
                options: {
                    backdrop: 'static',
                    controllerAs: 'vm'
                },
            ...
            success: {
                templateUrl: 'fm/modals/template/success.html',
                controllerName: 'fmModalsCommonController',
                options: {
                    backdrop: 'static',
                    controllerAs: 'vm'
            ...
            warning: {
                templateUrl: 'fm/modals/template/warning.html',
                controllerName: 'fmModalsCommonController',
                options: {
                    backdrop: 'static',
                    controllerAs: 'vm'
            ...
            error: {
                templateUrl: 'fm/modals/template/error.html',
                controllerName: 'fmModalsCommonController',
                options: {
                    backdrop: 'static',
                    controllerAs: 'vm'
            ...
            confirm: {
                templateUrl: 'fm/modals/template/confirm.html',
                controllerName: 'fmModalsCommonController',
                options: {
                    backdrop: 'static',
                    controllerAs: 'vm'
    ...
    modals.factory('fmModalsFactory', fmModals);
    fmModals.$inject = ['$uibModal', 'fmModalsConfig'];
    function fmModals($uibModal, fmModalsConfig) {
        var factory = {
            info: info,
            success: success,
            warning: warning,
        ...
        var config = {
            defaults: fmModalsConfig.defaults,
            info: fmModalsConfig.info,
            success: fmModalsConfig.success,
            warning: fmModalsConfig.warning,
            error: fmModalsConfig.error,
            confirm: fmModalsConfig.confirm
        };
        ...
        function resolveParams(type, args) {
            ...
            return { inputs: inputs, options: resolveOptions(type, options) };
        }
        function resolveOptions(type, optionsP) {
            var options = config[type].getOptions();
            angular.forEach(optionsP, function(item, index) {
                options[index] = optionsP[index];
            });
            return options;
        }
    ...
    function run($templateCache) {
        $templateCache.put('fm/modals/template/info.html', '<div class="modal-header"><i class="fa fa-info-circle fa-2x pull-left text-info"></i><h4 class="modal-title">{{ vm.title }}</h4></div><div class="modal-body bg-info"><p>{{ vm.message }}</p></div><div class="modal-footer"><button class="btn btn-primary" type="button" ng-click="vm.close()">{{ vm.closeButtonLabel }}</button></div>');
        $templateCache.put('fm/modals/template/success.html', '<div class="modal-header"><i class="fa fa-check-circle fa-2x pull-left text-success"></i><h4 class="modal-title">{{ vm.title }}</h4></div><div class="modal-body bg-success"><p>{{ vm.message }}</p></div><div class="modal-footer"><button class="btn btn-primary" type="button" ng-click="vm.close()">{{ vm.closeButtonLabel }}</button></div>');
        $templateCache.put('fm/modals/template/warning.html', '<div class="modal-header"><i class="fa fa-exclamation-circle fa-2x text-warning pull-left"></i><h4 class="modal-title">{{ vm.title }}</h4></div><div class="modal-body bg-warning"><p>{{ vm.message }}</p></div><div class="modal-footer"><button class="btn btn-primary" type="button" ng-click="vm.close()">{{ vm.closeButtonLabel }}</button></div>');
        $templateCache.put('fm/modals/template/error.html', '<div class="modal-header"><i class="fa fa-times-circle fa-2x text-danger pull-left"></i><h4 class="modal-title">{{ vm.title }}</h4></div><div class="modal-body bg-danger"><p>{{ vm.message }}</p></div><div class="modal-footer"><button class="btn btn-primary" type="button" ng-click="vm.close()">{{ vm.closeButtonLabel }}</button></div>');
        $templateCache.put('fm/modals/template/confirm.html', '<div class="modal-header"><i class="fa fa-question-circle fa-2x pull-left"></i><h4 class="modal-title">{{ vm.title }}</h4></div><div class="modal-body"><p>{{ vm.message }}</p></div><div class="modal-footer"><button class="btn btn-default" type="button" ng-click="vm.close(false)">{{ vm.cancelButtonLabel }}</button><button class="btn btn-primary" type="button" ng-click="vm.close(true)">{{ vm.confirmButtonLabel }}</button></div>');
    }

    modals.controller('fmModalsCommonController', fmModalsCommonController);
    fmModalsCommonController.$inject = ['$uibModalInstance', 'inputs'];
    function fmModalsCommonController($uibModalInstance, inputs) {
        var vm = this;
